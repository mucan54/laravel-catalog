...
class Relation extends Field
{
    ...
    public function fromModel(string $model, string $name, string $key = null): self
    {
        ...
        return $this->addBeforeRender(function () use ($model, $name, $key) {
            ...
            $scope = $this->get('myscope', 'handler');
            $scopeParameter = $this->get('scopeParameters', 'handler');
            ...
            $value = collect($value)->filter(function ($value) use($scopeParameter,$scope) { 
                return $value->$scope == $scopeParameter; 
            })->values();
    ...
    public function fromClass(string $class, string $name, string $key = 'id'): self
    {
        ...
        return $this->addBeforeRender(function () use ($class, $name, $key) {
            ...
            $class = $class->{$scope}($scopeParameter);
    ...
    public function applyMyScope(string $scope): self
    {
        $scope = lcfirst($scope);
        $this->set('myscope', $scope);
        return $this;
    }

    public function scopeParameters(string $scope): self
    {
        $this->set('scopeParameters', $scope);
